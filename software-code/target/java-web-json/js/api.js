...
(function (global) {
  ...
  function resolveApiBase() {
    if ((location.protocol || "").toLowerCase() === "file:") {
      throw new Error("Please open the app from Tomcat URL, not a local file path.");
    }
  ...
  function request(method, path, bodyObj) {
    ...
    return fetch(apiUrl(path), opts).then(function (res) {
      ...
      if (msg && msg.indexOf("Tomcat URL") !== -1) {
        throw err;
      }
      // Keep backend business errors (e.g. wrong password) unchanged.
      if (err && err.name !== "TypeError") {
        throw err;
      }
      throw new Error("Network request failed. Please open via http://localhost:8080/java-web-json/ ...");
    });
  }
